...
地址解析：
  "1.1.1.1:53"                          → UdpUpstream  （走 mirage 隧道 → server 端 plain TCP/UDP 转发）
  "tls://1.1.1.1:853"                   → DotUpstream  （mirage 隧道 → TLS 1.2+ → 长度前缀 DNS）
  "https://1.1.1.1/dns-query"           → DohUpstream  （mirage 隧道 → TLS → HTTP/1.1 POST application/dns-message）

约束：
  - DoH URL 暂只支持 **host = IP literal**（防 bootstrap 死循环：DNS 服务自己需要先解析 DNS 服务器的域名）
  - 所有 upstream 都共用同一条 mirage 隧道做底层；TLS 由客户端做端到端
...
class UdpUpstream(Upstream):
    """
    DNS-over-TCP via mirage tunnel。**实际上服务端再发出去仍是 plain TCP/53**，
    所以这是 plaintext DNS（依赖 mirage 隧道做线缆加密，不防上游运营商）。
...
class DotUpstream(Upstream):
    """
    DoT：mirage 隧道 → server 端 plain TCP 到 host:853 → 本地 TLS 客户端做端到端
    TLS 1.2+ 握手 → 长度前缀 DNS。
...
class DohUpstream(Upstream):
    """
    DoH：mirage 隧道 → server 端 plain TCP 到 host:443 → 本地 TLS + HTTP/1.1
