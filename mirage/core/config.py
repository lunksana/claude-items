...
_INBOUND_TYPES = ("socks5", "http", "mixed")
...
def _validate_inbounds(inbounds: Any) -> None:
    ...
    for i, ib in enumerate(inbounds):
        ...
        t = ib.get("type")
        if t not in _INBOUND_TYPES:
            raise ConfigError(
                f"inbounds[{i}]: type '{t}' not supported "
                f"(allowed: {', '.join(_INBOUND_TYPES)})"
            )
...
def _project_v1_to_legacy_keys(raw: dict) -> None:
    ...
    """
    # inbounds[*] (socks5 / mixed) → cfg['socks5_host'] / ['socks5_port']
    # 兼容下游仍读老顶层 key 的代码（如 UDP relay 默认绑定地址）。mixed 包含 SOCKS5
    # 路径，也需要这套投射。HTTP-only 入口的 listen 不投到 socks5_host（HTTP 没有 UDP）
    inbounds = raw.get("inbounds") or []
    for ib in inbounds:
        if ib.get("type") not in ("socks5", "mixed"):
            continue
