"""
mirage 私有端点（/mirage/*）。Clash 标准之外的诊断数据，给"mirage 面板"
或 curl 排查用。Yacd 不读这些，不影响兼容性。

  GET /mirage/pool       —— 每个 outbound 的 BrutalPool 实时状态
  GET /mirage/timesync   —— 当前 offset / 上次同步源 / 漂移
  GET /mirage/geo        —— geosite/geoip 缓存元数据（来自 meta.json）
"""
...
def register(router: Router, ctx: APIContext) -> None:
    # 主路径
    router.add("GET", "/mirage/pool",     _pool)
    router.add("GET", "/mirage/timesync", _timesync)
    router.add("GET", "/mirage/geo",      _geo)
    router.add("GET", "/mirage/cache",    _cache)
    # Legacy alias（0.4.35 重命名前的 /pyrealiy/* 路径，保留兼容）
