...
class HealthCheck:
    """所有 pyrealiy outbound 共用一个扫描 task"""
    ...
    def __init__(
        ...
        self._targets = [o for o in outbounds if isinstance(o, PyrealiyOutbound)]
    ...
    def start(self) -> None:
        if self._task is None and self._targets:
            ...
            logger.info("Health check started: %d pyrealiy outbound(s), every %.0fs, stale=%.0fs",
    ...
    async def _probe_one(self, o: PyrealiyOutbound) -> None:
        try:
            ms = await o.probe()
            if ms is not None:
