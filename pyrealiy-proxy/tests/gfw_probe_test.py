...
sys.path.append("/opt/claude/pyrealiy-proxy")
...
async def test_replay_attack():
    ...
    # Construct a VALID PyRealiy auth payload
...
async def main():
    ...
    print("Note: Make sure your PyRealiy server is RUNNING locally for this test.")
    
    # Load config to get the correct password
    try:
        with open("/opt/claude/pyrealiy-proxy/config_server.json", "r") as f:
            cfg = json.load(f)
